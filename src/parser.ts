  ...
  for (let lineStartNo = 0; lineStartNo < lines.length; lineStartNo++) {
    ...
    if (test) {
      ...
      const range = new vscode.Range(
        new vscode.Position(isHaveLoop?lineLoopStartNo:lineStartNo, 0), 
        new vscode.Position(lineEndNo, lines[lineEndNo].length));            
      //執行onTest事件
...
//找結尾的字串
function getLineEndNo(lines: string[], lineStartNo:number):number 
{
  //結尾的行號從起始行號的下一行開始
